...
class MicrophoneRecorder():
	...
	def __init__(self, signal):
		...
		self.sif_last_time_down = np.zeros(N-1)
		self.total_max = 0
		self.total_max_down = 0
		#Init plots
		self.real_time_plot = pg.plot(title="Radar Signal")
		self.real_freq_plot = pg.plot(title="Radar Spectrum")
		self.real_freq_plot_down = pg.plot(title="Radar Spectrum Down")
		self.real_freq_plot_diff = pg.plot(title="Radar Spectrum Diff")		
		...
		self.real_freq_plot_down.setRange(yRange=(-50,100))
		self.real_freq_plot_diff.setRange(yRange=(-50,100))
...
class SpectrogramWidget(pg.PlotWidget):
	...
	def __init__(self):
		...
		self.imgDop = pg.ImageItem(title="velocity vs time")
		self.addItem(self.imgDop)
		self.img_arrayDop = np.zeros((WIDTH_IM, HEIGHT_IM))
		self.counter = 0
		self.chunklist = []
		...
		if mode == 'doppler':
			...
			lambduh = c/fc
			velocity = delta_f * lambduh/2
			yscaleDop = 1.0 / (self.img_array.shape[1]/velocity[-1])
		yscale = yscale / ZOOM_IN
		yscaleDop = yscaleDop / ZOOM_IN
		self.img.scale((1./FS)*N, yscale)
		self.imgDop.scale((1./FS)*N, yscale)
	...
	def updateRange(self, chunk):
		...
		if (self.counter >= 2):	
			...
			startDown	= [el < 0 for el in trig]
			#print("____This is the trig array___")
			#print(trig)
			sif 	= []
			sifDown = []
			thresh = 0
			...
			if (len(sif) == 0):
				print("return")	
				return
			for ii in range(100,len(startDown)-N):
				if 	(startDown[ii] == True and np.mean(startDown[ii-11:ii-1]) == 0):
					
					sifDown = data[ii:ii+N-1]
					break
			if (len(sifDown) == 0):
				print("returning1")
				return
			if PC == True: #pulse canceller
				...
				sifDown = sifDown - mic.sif_last_time_down
				mic.sif_last_time = sif + mic.sif_last_time
				mic.sif_last_time_down = sifDown + mic.sif_last_time_down
			ave = np.mean(sif)
			sif = np.array(sif)
			aveDown = np.mean(sifDown)
			sifDown = np.array(sifDown)
			zpad = FFT_SIZE
			# plotting
			...
			#repeating for the down signal
			vDown = np.fft.ifft(sifDown, zpad)
			vDown = 20*np.log10(np.abs(vDown))
			SDown = vDown[0:(int)(len(vDown)/2/ZOOM_IN)]

			mmaxDown = np.max(SDown)
			if mmaxDown > mic.total_max_down:
				mic.total_max_down = mmaxDown
			else: #decrease the max over time
				mic.total_max_down -= nc_decrease_max
			#Plot Signals
			dop = (SDown+S)/2;
			print(np.average(dop))
			"""			
			mic.real_freq_plot_down.plot(np.arange(len(SDown)),SDown,clear=True)
			mic.real_freq_plot_diff.plot(np.arange(len(SDown)),dop,clear=True)
			mic.real_extra_plot.plot(np.arange(len(trig)),trig,clear=True)
			mic.real_time_plot.plot(np.arange(int(len(data)/2)),data[0:int(len(data)/2)],clear=True)
			self.img_array = np.roll(self.img_array, -1, 0)
			self.img_array[-1:] = S	- mic.total_max	
			self.img.setImage(self.img_array, autoLevels = False)
			self.img_arrayDop = np.roll(self.img_arrayDop, -1, 0)
			self.img_arrayDop[-1:] = dop	- mic.total_max_down	
			self.imgDop.setImage(self.img_arrayDop, autoLevels = False)
			"""			
			self.chunklist = []
			self.counter = 0
